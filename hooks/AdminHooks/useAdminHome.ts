...
export function useAdminHome() {
    ...
    const cargarResumen = useCallback(async () => {
        ...
        try {
            ...
            // (El tamaño del arreglo gigante de usuarios nos dice cuántos son en total)
            setTotalUsuarios(data.usuarios.length)
            // (Filtramos solo a los que dicen cliente y contamos cuántos son)
            setTotalClientes(data.usuarios.filter((user: any) => user.rol === 'cliente').length)
            // (Filtramos solo a los operadores y los contamos)
            setTotalOperadores(data.usuarios.filter((user: any) => user.rol === 'operador').length)
            // (Filtramos a los jefes y los contamos)
            setTotalAdmins(data.usuarios.filter((user: any) => user.rol === 'admin').length)

            // (Asignamos los demás números totales a sus respectivos estados)
    ...
    return {
        ...
        cargarResumen,
        handleLogout,
        // (Pequeñas funciones de atajo para que los botones te lleven directo a cada sección)
