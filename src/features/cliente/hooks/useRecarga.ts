...
export function useRecarga() {
    ...
    async function handleRecargar() {
        ...
        try {
            ...
            // (Guardamos referencia al cupón usado ANTES de limpiar el estado)
            const cuponUsadoId = cuponSeleccionado?.id ?? null

            // (Limpiamos el cupón seleccionado del estado local)
            setCuponSeleccionado(null)

            // (Si hubo cupón, verificamos que haya quedado marcado como usado en la BD)
            // (Si sigue disponible = la política RLS está bloqueando el UPDATE en Supabase)
            if (cuponUsadoId) {
                const { data: cuponVerificado } = await CuponesService.obtenerCuponesDisponibles(usuarioId, 'cliente')
                const sigueDisponible = (cuponVerificado ?? []).some((c: any) => c.id === cuponUsadoId)
                if (sigueDisponible) {
                    console.warn('[useRecarga] El cupón sigue disponible tras la recarga. Verifica la política RLS UPDATE en Supabase para la tabla "cupones".')//fuckingcupones
                }
            }

            // (Recargamos los datos del panel para que el cupón desaparezca de la vista)
            await cargarDatosRecarga()
