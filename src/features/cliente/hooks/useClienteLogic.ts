...
export function useClienteLogic() {
    ...
    // (Recargamos los datos cada vez que la pantalla entra en foco)
    // (Esto es clave para que el cupón desaparezca apenas el cliente vuelva del pago)
    // (useFocusEffect > useEffect porque el panel queda montado en el stack de navegación)
    useFocusEffect(
        useCallback(() => {
            cargarDatosCliente()
        }, [cargarDatosCliente])
