...
const AdminCuponSchema = z.object({
    ...
    propietario_rol: z.enum(['cliente', 'operador', 'admin']).nullable().optional(),
...
export const AdminService = {
    ...
    // (Función del dashboard que cuenta cuántos hay de todo usando COUNT de Supabase,
    //  evitando el límite de 1000 filas que PostgREST aplica por defecto)
    obtenerTotalesDashboard: async () => {
        // (Usamos { count: 'exact', head: true } para pedir solo el conteo sin bajar filas)
        const [totalQuery, clientesQuery, operadoresQuery, adminsQuery, pagosQuery, recargasQuery, cuponesQuery] = await Promise.all([
            supabase.from('perfiles').select('*', { count: 'exact', head: true }),
            supabase.from('perfiles').select('*', { count: 'exact', head: true }).eq('rol', 'cliente'),
            supabase.from('perfiles').select('*', { count: 'exact', head: true }).eq('rol', 'operador'),
            supabase.from('perfiles').select('*', { count: 'exact', head: true }).eq('rol', 'admin'),
            supabase.from('pagos_qr').select('*', { count: 'exact', head: true }).eq('estado', 'aprobado'),
            supabase.from('recargas').select('*', { count: 'exact', head: true }).eq('estado', 'aprobada'),
            supabase.from('cupones').select('*', { count: 'exact', head: true }),
        ])

        // (Devolvemos los conteos exactos directamente, sin iterar arreglos en el cliente)
        return {
            totalUsuarios:   totalQuery.count     ?? 0,
            totalClientes:   clientesQuery.count  ?? 0,
            totalOperadores: operadoresQuery.count ?? 0,
            totalAdmins:     adminsQuery.count     ?? 0,
            totalPagos:      pagosQuery.count      ?? 0,
            totalRecargas:   recargasQuery.count   ?? 0,
            totalCupones:    cuponesQuery.count    ?? 0,
            error: totalQuery.error || clientesQuery.error || operadoresQuery.error ||
                   adminsQuery.error || pagosQuery.error || recargasQuery.error || cuponesQuery.error
        }
    },
