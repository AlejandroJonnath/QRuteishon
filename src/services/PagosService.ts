import { z } from 'zod';
import type { PagoQr } from '@/features/cliente/hooks/usePagarQr';
...
export const PagosService = {
    ...
    procesarPagoBilletera: async (
        ...
        // (Paso 4 opcional: Si el cobro tenía cupón aplicado lo quemamos usando el servicio centralizado)
        if (pago.cupon_codigo) {
            // (Primero buscamos el ID del cupón por su código, ya que el QR solo guarda el código de texto)
            ...
            if (cuponData?.id) {
                await CuponesService.marcarCuponUsado(cuponData.id, pago.id)
            } else {
                console.log('[PagosService] No se encontró cupón disponible con código:', pago.cupon_codigo)
            }
        }
